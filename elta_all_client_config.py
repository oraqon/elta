#!/usr/bin/env python3
"""
ELTA All Client Configuration - Connect to all ports as clients
Based on TDP log analysis - try connecting to port 30073 as client
"""
...
class EltaAllClientConfig:
    def __init__(self):
        # Network configuration - try ALL ports as clients
        self.elta_simulator_ip = 'localhost'
        
        # All ports as TCP CLIENTS (we connect to simulator)  
        self.tcp_client_ports = [23004, 30080, 6072, 9966, 30073]  # Added 30073!
    ...
    def start_tcp_client(self, port):
        ...
        while self.running:
            try:
                ...
                # Send System Control after connection (but not to port 30073)
                if port != 30073:  # Don't send control to 30073 - might be different purpose
                    time.sleep(1)
                    self.send_system_control(port)
                else:
                    print(f"[{datetime.now()}] 🎯 Port 30073 connected - waiting for simulator messages...")
                ...
                if self.running:
                    ...
                    time.sleep(3)  # Shorter retry time
            finally:
    ...
    def send_system_control_operate(self):
        ...
        # Send via first available TCP client connection (exclude 30073)
        for port, sock in self.tcp_clients.items():
            if port == 30073:  # Skip 30073 - might be for different purpose
                continue
    ...
    def send_acknowledge(self, original_seq_num):
        ...
        # Try to send acknowledge via any available TCP client connection (exclude 30073)
        for port, sock in self.tcp_clients.items():
            if port == 30073:  # Skip 30073
                continue
    ...
    def start(self):
        """Start the all-client configuration"""
        self.running = True
        
        print("🚀 ELTA All-Client Configuration")
        print("=" * 70)
